...
$rentalRecords = []; // Initialize an empty array for rental records

// Check if the SSN has been passed to this script via POST
if (isset($_POST['ssn'])) {
    $ssn = $_POST['ssn']; // You may want to sanitize this input

    // Create database connection
    $cn = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
    if (!$cn) {
        // Handle error - notify admin or log to a file or database
        die('Error connecting to database: ' . mysqli_connect_error());
    }

    // Prepare the SQL statement to select rental records based on SSN
    // Always use prepared statements to prevent SQL injection
    $stmt = mysqli_prepare($cn, "SELECT * FROM rental WHERE user_ssn = ?");
    ...
    if ($result) {
        ...
        $rentalRecords = mysqli_fetch_all($result, MYSQLI_ASSOC);
    } else {
        // Handle error - notify admin or log to a file or database
    ...
    // SSN was not passed to this script
    header("location:displayAllUsers.php?msg=No SSN provided");
    exit;
}
...
<body>
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <!-- Admin Account Navbar & Sidebar here -->

            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">Rental History for SSN: <?php echo ($ssn); ?></h1>
                </div>
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php if (!empty($rentalRecords)) { ?>
                            <?php foreach ($rentalRecords as $record) { ?>
                                <tr>
                                    <td><?php echo htmlspecialchars($record['reservation_date']); ?></td>
                                    <td><?php echo htmlspecialchars($record['pick_up_date']); ?></td>
                                    <td><?php echo htmlspecialchars($record['return_date']); ?></td>
                                    <td><?php echo htmlspecialchars($record['plate_no']); ?></td>
                        ...
                        <?php } else { ?>
                            <tr>
                                <td colspan="6">No rental records found for this User.</td>
                            </tr>
                        <?php } ?>
                    </tbody>
                </table>
            ...
            <!-- Include Bootstrap JS and other JavaScript files -->
