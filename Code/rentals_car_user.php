...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Check if the 'mySelect' field exists in the POST request
    if (isset($_POST['datePicked'])) {
        if (empty($_POST['datePicked'])) {
            $data = rental::getCar_UserRentals();
        } else {

            $datePicked = $_POST['datePicked'];
            echo $datePicked;
            $data = rental::getCar_User_dateRentals($datePicked);
        }


        // Process the selected value (here, we are just echoing it)
        // echo "Selected value";
...
<head>
    ...
    <title>Admin - Filter Rentals</title>
...
<body>
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi" viewBox="0 0 16 16" id="person-add">
    </svg>
    ...
    <header class="navbar sticky-top bg-dark flex-md-nowrap p-0 shadow" data-bs-theme="dark">
        ...
        <ul class="navbar-nav flex-row d-md-none">
            <li class="nav-item text-nowrap">
                <button class="nav-link px-3 text-white" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSearch" aria-Scontrols="navbarSearch" aria-expanded="false" aria-label="Toggle search">
                    <svg class="bi">
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                ...
                <form id="rentalForm" action="rentals_car_user.php" method="POST" style="display: none;">
                    <div id="dateInputs" class="form-group">
                        <label for="startDate">Pick a Date:</label>
                        <input type="text" class="form-control" id="datePicked" name="datePicked">
                    </div>
                ...
                <script>
                    $(document).ready(function() {
                        // Show date inputs and form on page load
                        $('#rentalForm').show();
                        $('#dateInputs').show();
                ...
                <table class="table table-image">
                    ...
                    <tbody>
                        ...
                        if ($num_rows == 0) {
                            ...
                            foreach ($data as $d) {
                                echo '<tr>';
                                echo '<td><p><strong>Plate #:</strong> ' . $d["plate_no"] . ' 
                            <br>
                            <strong>Model:</strong> ' . $d["model"] . '
                            <br>
                            <strong>Type:</strong> ' . $d["car_type"] . '
                            <br>
                            <strong>Color:</strong> ' . $d["color"] . '
                            <br>
                            <strong>Capacity:</strong> ' . $d["capacity"] . '
                            <br>
                            <strong>Price/Day:</strong> ' . $d["price_per_day"] . '
                            <br>
                            <strong>Location:</strong> ' . $d["city"] . ', ' . $d["country"] . '
                            <br>
                            <strong>Car status:</strong> ' . $d["status"] . '
                            </p>
                            </td>';
                                echo '<td><p><strong>Name:</strong> '  . $d["fname"] . ', ' . $d["lname"] . '
                            <br>
                            <strong>SSN:</strong> ' . $d["ssn"] . '
                            <br>
                            <strong>Email:</strong> ' . $d["email"] . '
                            <br>
                            <strong>Phone no.:</strong> ' . $d["phone_no"] . '
                            <br>
                            <strong>Address:</strong> ' . $d["address"] . '
                            </p>
                            </td>';
                                echo '<td>' . $d["reservation_date"] . '</td>';
                                echo '<td>' . $d["pick_up_date"] . '</td>';
                                echo '<td>' . $d["return_date"] . '</td>';
                                echo '<td>' . $d["card_no"] . '</td>';
                                echo '<td>' . $d["total_amount"] . '</td>';
                                echo '<td>' . $d["rental_status"] . '</td>';
                                echo '</tr>';
                            }
                        }
                        ?>
