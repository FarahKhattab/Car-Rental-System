...
<head>
    ...
    <title>Display Daily Payments</title>
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div
                    ...
                    <body>
                        ...
                        <div class="container">
                            <h1 class="h2">DAILY PAYMENTS</h1>
                            <p>within:</p>
                            <?php
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            // Check if startDate and endDate are set and not empty
            if (isset($_POST["startDate"]) && isset($_POST["endDate"]) && !empty($_POST["startDate"]) && !empty($_POST["endDate"])) {
                ...
                echo "Start Date or End Date is missing.";
            }
        }
        ?>
...
// SQL query to get rentals between startDate and endDate
$sql = "SELECT reservation_date, total_amount FROM rental WHERE reservation_date BETWEEN '$startDate' AND '$endDate'";

$result = $conn->query($sql);

if ($result->num_rows > 0) {
    $dailySum = []; // Array to store daily sums

    // Initialize daily sums to zero for each day
    $currentDate = strtotime($startDate);
    $endDateTimestamp = strtotime($endDate);
    while ($currentDate <= $endDateTimestamp) {
        $dailySum[date('Y-m-d', $currentDate)] = 0;
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // Increment daily sum for the reservation date
        $dailySum[$reservationDate] += $totalAmount;
    }
     $i=0;
    // Print daily sums
    foreach ($dailySum as $date => $sum) {
        echo "Date: $date, Total Amount: $sum <br>";
        $date2[$i] = $date;
        $sum2[$i] = $sum;  
        $i++;
    ...
    echo "No rentals found between $startDate and $endDate";
}
...
?>
                            ...
                            <body>
                                ...
                                <?php
  // Assuming $dates and $dailySum contain your date and sum data
//   $dates = ["2023-01-01", "2023-01-02", "2023-01-03"]; // Replace with your date array
//   $dailySum = [10, 20, 15]; // Replace with your sum array
  ?>

                                <script>
                                // Get PHP data into JavaScript
                                const dates = <?php echo json_encode($date2); ?>;
                                const dailySum = <?php echo json_encode($sum2); ?>;
                                ...
                                const myChart = new Chart(ctx, {
                                    ...
                                    data: {
                                        ...
                                        datasets: [{
                                            label: 'Daily Sum',
                                            data: dailySum, // Use dailySum as the data
                                            borderColor: 'blue',
